...
function CommentList({ data, id }) {
  let date = `${new Date(data.post_date).getFullYear()}.${
    new Date(data.post_date).getMonth() + 1
  }.${new Date(data.post_date).getDate()}`;
  return (
    <>
      <div className={styles.comment_list}>
        {id % 2 ? (
          <RightCommentItem data={data} post_date={date} />
        ) : (
          <LeftCommentItem data={data} post_date={date} />
        )}
      </div>
    </>
  );
      ...
      <div className={styles.wrapper}>
        <div className={styles.comment_block}>
          <img
            ...
            src="https://user-images.githubusercontent.com/37580034/89393019-37606180-d745-11ea-87f7-93a71ca3b152.png"
          />
          <div className={styles.comment_dialog}>
            <div
              className={classNames({
                [styles.link]: true,
                [styles.link_adsila]: true,
              })}
            >
              <a className={styles.link_item} href={data.link} target="_black">
                <span className={styles.link_item_name}>
                  {data.title}
    ...
    <>
      <div className={styles.wrapper}>
        <div className={styles.comment_block_r}>
          ...
          <img
            className={styles.comment_image_r}
            src="https://user-images.githubusercontent.com/37580034/89393019-37606180-d745-11ea-87f7-93a71ca3b152.png"
          />
        </div>
      </div>
  ...
  return (
    <>
      ...
      <div className={styles.box}>
        {reviewData[program]?.map((v, idx, id) => (
