...
import { getYearlyCalendar } from "../utils/api";
    ...
    <>
      <div
        className={classNames({
          [styles.gantt__row]: true,
          [styles.gantt__row__months]: true,
        })}
      >
        <div className={styles.gantt__row__first}></div>
        {month.map((v, idx) => (
          <span key={idx}>{v}월</span>
        ))}
      </div>
      <div
        className={classNames({
          [styles.gantt__row]: true,
          [styles.gantt__row__lines]: true,
        })}
      >
        {month.map((v, idx) =>
          v === monthValue ? (
            <span className={styles.current_marker} key={idx}></span>
          ) : (
            <span key={idx}></span>
          )
        )}{" "}
        {/* 12칸으로 나누고, 현재 월이면 배경색 칠함. */}
      </div>
    </>
  );
}
...
function ProgramList() {
  ...
  const fetchData = async () => {
    const res = await getYearlyCalendar();
    setData(res);
  };
  useEffect(() => {
    fetchData();
...
function GisuList({ data }) {
  return (
    <>
      {data.gisuData.map((v, idx) => (
        <StepList data={v} key={idx} title={data.title} link={data.idName} />
      ))}
    </>
  );
}

function StepList({ data, title, link }) {
  {
    /* Calendar의 Row */
  }
  let title_and_gisu = (
    <span>
      ...
      <br />
      <br />
      <span className={styles.gisu}>{data.gisu}</span>
    </span>
  );
  let title_only = (
    <span>
    ...
    <>
      <div className={styles.gantt__row}>
        <div className={styles.gantt__row__first}>
          ...
          {/* 각 Row의 첫 번째 Column */}
          {(() => {
            if (data.visible === 1 && title === "42SEOUL ")
              return title_and_gisu;
            else if (data.visible === 1) return title_only;
            else if (data.visible > 1) return gisu_only;
          })()}
        </div>
        <ul className={styles.gantt__row__bars}>
          {data.step.map((v, idx, id) => (
            <StepListItem data={v} key={idx} id={data.step.length - 1 - idx} />
          ))}
        </ul>
      </div>
    </>
...
function StepListItem({ data, id }) {
  ...
  {
    /* 두 달이 넘어가는 일정은 hover 시 자기 자신의 크기를 가짐 */
  }
  let next_year =
    new Date(data.endDate).getFullYear() > new Date().getFullYear() ? 1 : 0;
  {
    /* 내년으로 이어지는 일정은 오른쪽 border-radius를 0으로 함 */
  }
    ...
    <>
      {(() => {
        if (oversize_list === 0 && next_year === 0)
          return (
            <li
              className={classNames({ [styles.calendar_entry]: true })}
              style={{
            ...
            >
              ...
              <span
                className={styles.calendar_entry__date}
          ...
          return (
            <li
              className={classNames({
                [styles.calendar_entry]: true,
                [styles.oversize_list]: true,
              })}
              style={{
                gridColumn: gridColumn,
                backgroundColor: backgroundColor,
...
export default function YearlyCalendar() {
  return (
    <>
      <Header>
        <h1 className={styles.webheader}>
          {new Date().getFullYear()}년 부트캠프
        </h1>
        <p className={styles.websubtitle}>
          무료 부트캠프 일정을 한 눈에 확인하세요.
        </p>
      </Header>
      <div
        className={classNames({
          [styles.wrapper]: true,
          [styles.landscape_only]: true,
        })}
      >
        <CTA />
